...
const Landing: React.FC = () => {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, (user) => {
      if (user) {
        const fullName = user.displayName ? user.displayName.split(" ").slice(0, 2).join(" ") : "User";
      ...
      {user ? (
        ...
        <>
          <button
            onClick={handleLogin}
            disabled={loading}
            className={`text-white text-xl mt-[13vh] lg:hidden tracking-tighter ${loading ? "opacity-50 cursor-not-allowed" : ""}`}
          >
          ...
          <button
            onClick={handleLogin}
            disabled={loading}
            className={`text-white text-2xl mt-[10vh] hidden lg:block ${loading ? "opacity-50 cursor-not-allowed" : ""}`}
          >
            {loading ? "Signing In..." : "<Sign In with Google>"}
          </button>
        </>
