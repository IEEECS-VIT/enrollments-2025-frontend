        ...
        <div className="flex flex-col gap-8 z-20">
            ...
            {faqs.map((faq, index) => (
                ...
                <div className={`rounded-3xl ${
                    expandedIndex === index ? 'border-2 border-orange-400 text-center' : 'border-2 border-white text-center'
                } p-0.5`}>
                    <button
                    className="text-center p-4 bg-black rounded-3xl text-white"
                    onClick={() => setExpandedIndex(expandedIndex === index ? null : index)}
                    >
                    <p className="font-press-start lg:text-[1.5rem] text-[10px] md:text-[13px]">{faq.question}</p>
                    </button>
                    {expandedIndex === index && (
                    <div className="bg-black p-4 border-t-2 border-orange-400 rounded-b-3xl">
                        <p className="font-press-start text-[10px] md:text-[13px] text-white text-center">{faq.answer}</p>
                    </div>
                    )}
                </div>
                </div>
