        ...
        <div className="text-white min-h-screen flex flex-col items-center justify-center font-press-start">
            <div className="border-2 border-white mt-[10vh] rounded-3xl w-[80%] lg:w-[70%] sm:h-[65vh] h-[75vh] flex flex-col items-center p-8">
                ...
                <div className="flex-col justify-start items-start leading-[2rem] md:leading-[3rem] text-[0.6rem] md:text-xl p-2">
                    <p className="mb-4">Character Name: <span className="block md:inline">{profileData?.name}</span></p>
                    <p className="mb-4">Mobile No.: <span className="block md:inline">{profileData?.mobile}</span></p>
                    <p className="mb-4">Email id: <span className="block md:inline">{profileData?.email}</span></p>
                    <p className="mb-4">Selected domain: 
    ...
    domainList.length > 0 && (
      <div key={key}>
        ...
        {index < Object.keys(profileData.domain).length - 1 && <br />}
            ...
            <p className="text-white text-[0.6rem] md:text-xl absolute md:bottom-10 bottom-5" onClick={handleSignOut}>Sign out</p>
        </div>
    );
}
